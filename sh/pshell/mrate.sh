...
maxNumber=$(wc -l survivors.dat | awk '{ print $1 ; }')
...
do
    #echo $x;
    maxSingle=$(less survivors.dat | grep ^$x[^0-9] | wc -l)	#for each timeslot find the number of available data
    less survivors.dat | grep ^$x[^0-9] > tempFile
